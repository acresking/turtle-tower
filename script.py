...
def generate_coins():
    for shelf in total_shelves_list:
        if shelf.number % 20 == 0: 
            coin_value = random.choice([2, 2, 5, 5, 5, 10])  
            coin_x = random.randint(shelf.rect.x, shelf.rect.x + shelf.width - 32)
            coin_y = max(0, shelf.rect.y - 32)  
...
def DrawScore():
    score_text = SCORE_FONT.render("Score: " + str(body.score), True, BLACK)  
...
def DrawTime(paused):
    if not paused:  
        ...
        time_text = TIME_FONT.render("Time: " + str(elapsed_time) + "s", True, BLACK)  
        time_rect = time_text.get_rect(midtop=(WIDTH // 2, 70))  
...
def ResetGame():
    ...
    pygame.mixer.stop() 
    pygame.mixer.Sound.play(GAMEPLAY_SOUND, loops=-1)  
        ...
        while game_running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    game_running = False
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if DrawPauseButton(game_paused).collidepoint(event.pos):  
